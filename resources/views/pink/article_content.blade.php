...
<div id="content-single" class="content group">
    <div class="hentry hentry-post blog-big group ">
        ...
        <div class="meta group">
            ...
            <p class="categories"><span>In: <a href="{{route('articlesCat',['cat_alias'=>$article->category->alias])}}" title="View all posts in {{$article->category->title}}" rel="{{$article->category->title}}">Design</a>, </span></p>
            <p class="comments"><span><a href="#comments" title="Comment on This is the title of the first article. Enjoy it.">{{count($article->comment) ? count($article->comment) : '0' }}  {{ Lang::choice('ru.comments',count($article->comment))}}</a></span></p>
        </div>
        <!-- post content -->
