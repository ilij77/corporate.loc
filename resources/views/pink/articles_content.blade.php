   ...
   @foreach($articles as $article)
            ...
            <div class="sticky hentry hentry-post blog-big group">
                ...
                <div class="meta group">
                    ...
                    <p class="categories"><span>In: <a href="{{route('articlesCat',['cat_alias'=>$article->category->alias])}}" title="{{$article->category->title}}" rel="category tag">{{$article->category->title}}</a>, <a href="#" title="View all posts in Romantic Vintage" rel="category tag">Romantic Vintage</a></span></p>
                    <p class="comments"><span><a href="{{route('articles.show',['alias'=>$article->alias])}}#respond" title="Comment on Section shortcodes &amp; sticky posts!">{{count($article->comment) ? count($article->comment) : '0'}} {{Lang::choice('ru.comments',count($article->comment))}}</a></span></p>
                </div>
                <!-- post content -->
 ...
 @for($i=1;$i<=$articles->LastPage();$i++)
     ...
     @endfor
            @if($articles->currentPage()!==$articles->LastPage())
                    <a href="{{$articles->url($articles->currentPage() +1)}}">{{Lang::get('pagination.next')}}</a></div>
                @endif
